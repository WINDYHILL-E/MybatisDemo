...
import org.yahaha.mybatis.datasource.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements DataSource {
    ...
    private PooledConnection popConnection(String username, String password) throws SQLException {
            ...
            synchronized (state) {
                ...
                if (!state.idleConnections.isEmpty()) {
                    ...
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        // 这是活跃连接数已经到了最大值
                        // 那就拿活跃连接里面最老那个，也就是列表的第一个
                        PooledConnection oldestActiveConnection = state.activeConnections.get(0);
                        long longestCheckoutTime = oldestActiveConnection.getCheckoutTime();
                        // 如果checkout时间过长，那就得把这个连接标记成过期的
        ...
        if (conn == null) {
            logger.debug("PooledDataSource: Unknown serve error condition. The connection pool returned a null connection.");
            throw new SQLException("PooledDataSource: Unknown serve error condition. The connection pool returned a null connection.");
        }
    ...
    public void forceCloseAll() {
        synchronized (state) {
            ...
            for (int i = state.activeConnections.size(); i > 0; i--) {
                try {
                    PooledConnection conn = state.activeConnections.get(i - 1);
                    conn.invalidate();
            ...
            for (int i = state.idleConnections.size(); i > 0; i--) {
                try {
                    PooledConnection conn = state.idleConnections.get(i - 1);
                    conn.invalidate();
